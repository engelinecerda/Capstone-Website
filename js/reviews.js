...
/* ============================================================
   FETCH REVIEWS — with fallback strategy
============================================================ */
async function fetchReviews() {
    console.log('[Reviews] Fetching reviews from Supabase...');

    // Step 1: Get reviews only (no join) — this avoids FK relationship issues
    ...
    // Step 2: Get unique user_ids and fetch profile info separately
    ...
    if (profilesError) {
        console.warn('[Reviews] Could not fetch profiles, showing reviews without names:', profilesError);
        // Continue without profile data
        return reviews.map(r => ({ ...r, profile: null }));
    }

    // Step 3: Merge profiles into reviews
    const profileMap = {};
...
function renderReviews() {
    ...
    let filtered = showRatingOnly
        ? [...allReviews]  // include everything
        : allReviews.filter(r => r.comment && r.comment.trim().length > 0);  // written only
    ...
    list.innerHTML = visible.map(r => {
        ...
        const badgeHtml = hasComment
            ? `<span class="review-card__badge">
                   <i class="fa-solid fa-circle-check"></i> Verified Reservation
               </span>`
            : `<span class="review-card__rating-only-badge">
...
function setupListeners() {
    ...
    document.getElementById('toggleRatingOnly').addEventListener('change', e => {
        ...
        visibleCount = REVIEWS_PER_PAGE;  // reset pagination
        renderReviews();
    });
}

/* ============================================================
   INIT — guarantees loading spinner is removed
============================================================ */
async function init() {
    setupListeners();
    ...
    try {
        ...
        console.log('[Reviews] Raw data sample:', raw.slice(0, 2));
               ...
               // Apply all moderation layers (1, 2, 4) + log reasons for debugging
