-- Reservation Form Step 1 rework — the category-selection step now filters
-- packages by package.location_type client-side, but nothing server-side
-- ever checked that the submitted location_type is actually compatible with
-- the submitted package_id. A tampered client request could pair an
-- onsite-only package with location_type='offsite' (or vice versa) and the
-- existing enforce_reservation_capacity() trigger (from
-- 20260731_reservation_package_integrity.sql) would accept it, since it only
-- checks guest range / price floor / active status / scheduling overlap.
--
-- Extends the same trigger function (same pattern as 20260731) rather than
-- adding a second trigger, since it already does the one `package` lookup
-- this needs.
-- ─────────────────────────────────────────────────────────────────────────────
