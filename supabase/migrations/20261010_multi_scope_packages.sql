-- Multi-scope packages — a "Plus" package (e.g. a combo package that uses
-- both VIP and Main Hall for a bigger event) needs to occupy MORE THAN ONE
-- booking scope at once. package.booking_scope / reservations.booking_scope
-- were single scalar values, so a package like that could only ever be
-- assigned ONE of the two rooms it actually uses — the other stayed
-- completely unprotected on the calendar (nothing stopped a separate
-- customer from booking it directly).
--
-- This migration turns booking_scope into a set (text[]) on both tables —
-- a normal package still gets exactly one element, same as today; a combo
-- package gets two. Every function that resolves/compares scope is updated
-- to treat "does reservation A conflict with reservation B" as "do their
-- scope SETS share any element" (Postgres's array `&&` overlap operator)
-- instead of scalar equality, and the daily-capacity check now walks each
-- scope in a combo package's set independently, since VIP and Main Hall
-- still have their own separate daily caps.
--
-- Existing packages are backfilled as single-element arrays (zero behavior
-- change). Any live "Plus" package that actually needs both rooms still
-- needs an admin to go re-save it with both boxes checked in the Bookable
-- Inventory UI — this migration has no way to know which existing rows
-- those are.
-- ─────────────────────────────────────────────────────────────────────────────
...
-- Every element must be one of the three known scopes, at least one element
-- when set at all, and 'offsite' never combines with an onsite scope (a
-- package is either onsite or offsite, never both at once).
alter table public.package
  add constraint package_booking_scope_check
  check (
    booking_scope is null
    or (
      array_length(booking_scope, 1) > 0
      and booking_scope <@ array['onsite_vip', 'onsite_main_hall', 'offsite']::text[]
      and not (
        'offsite' = any(booking_scope)
        and array_length(booking_scope, 1) > 1
      )
    )
  );
...
-- advance_notice_enforcement.sql) with only the scope handling changed to
-- sets (marked below) — package integrity, the discount-aware price floor,
-- and the advance-notice window are otherwise untouched.
...
-- ── 3. enforce_reschedule_capacity() — array-aware ──────────────────────────
-- Full body reproduced from its current source (20260920_package_extension_
-- hours.sql §8b) with the same scope-handling change as above; operating-
-- hours, advance-notice, and buffer logic are otherwise untouched.
...
-- ── 4. count_held_reschedule_conflicts() / count_held_extension_conflicts()
--    — internal scope comparison only, signature unchanged (both still take
--    a single p_scope; callers above now loop over a reservation's scope
--    set and call these once per element). ────────────────────────────────
...
-- ── 5. get_available_start_times() — internal scope comparison only,
--    signature unchanged (still takes a single p_scope). A combo package's
--    slot is only truly available where every one of its scopes reports
--    available — js/reservation_availability.js now calls this once per
--    scope in the package's set and ANDs the results, rather than this
--    function learning a second, array-typed signature. Full body
--    reproduced verbatim from its current source (20260920_package_
--    extension_hours.sql §8c) with only the one comparison changed. ──────
...
-- ── 6. get_extension_availability() / finalize_extension_on_fee_approval()
--    — array-aware. An extension on a combo reservation must keep BOTH of
--    its scopes free for the extended time, so "what's the next thing that
--    blocks this reservation from extending" now checks any reservation/
--    hold sharing ANY of this reservation's scopes, not just an exact
--    single-scope match. ─────────────────────────────────────────────────
